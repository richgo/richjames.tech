...
![TypeSafe security review program flow: repositories use local CodeGraph discovery or a file-walk fallback, while file, text and stdin inputs enter the coordinator directly. Relevance routing selects concurrent OWASP and CWE classifiers. Code buckets independent category probabilities into Pass, Review or Failed. Optional coding-agent review can resolve Review findings; errors remain Review. The printed report hides Pass findings, while repository JSON retains them.](/img/typesafe-security-program-flow.png)

[Open the full-size diagram](/img/typesafe-security-program-flow.png) · [Program flow and implementation](https://github.com/open-sdlc/typesafe-security-review#program-flow)

The diagram reflects the project's program flow as of 19 September 2026. Blue-grey boxes are deterministic code, gold boxes are TypeSafe judgements, and the purple branch is an optional coding-agent review. Repository scans index locally with CodeGraph (with a plain file-walk fallback), then process files in parallel. Single-file, text and stdin inputs enter the same coordinator directly. The router and selected classifiers receive the file's text, not the graph itself.

Routing is a model judgement too: code applies a relevance threshold to choose which classifiers run, then separate confidence bands to their category probabilities. By default, **Pass** is below 0.40, **Review** is 0.40–0.65 inclusive, and **Failed** is above 0.65. These thresholds are configurable. Pass findings are hidden from the printed table but retained in repository `--json` output.

Each [Noul](https://docs.typesafe.ai/primitives/noul) value is an independent probability that the answer to its yes/no question is **yes**. It is not a severity score or an overall confidence rating, and the category probabilities do not need to sum to one.

The project now also implements an **opt-in second opinion** with `--llm-review`. Review-band findings are sent to Copilot, Claude or Codex as file paths and finding metadata over stdin, rather than embedding file contents in the prompt. The agent runs in the repository with tool access and reads the flagged files itself. A strict fenced JSON verdict array reclassifies findings as Pass or Failed; CLI errors or unparseable responses leave them at Review. Without the flag, they stay at Review in the final report. This is deliberately off by default: it introduces a separate model bill and gives an agent access to potentially untrusted repository content.
...
This isn't a case for throwing away deterministic scanners. CodeGraph's file discovery is deterministic; the router's relevance check is a model judgement, and it can be wrong. Nor do narrow, calibrated judgements replace deep reasoning: a genuinely novel, multi-step business-logic exploit chain still deserves a reasoning model and a human having a proper look. The optional agent-review branch now provides one escalation path for Review-band findings, but it won't catch something the first pass missed entirely, and it doesn't replace human judgement.
